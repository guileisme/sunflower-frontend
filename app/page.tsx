...
const renderFactorIcon = (
  ...
  <div
    className={`w-14 h-14 ${bgColor} ${borderColor} rounded-2xl flex items-center justify-center ${color} border shadow-sm shrink-0 transition-all`}
  >
  ...
  const fatores = [
    {
      ...
      val: "Alta",
      icon: Sun,
      color: "text-sun-green-600",
      bgColor: "bg-green-100/50",
      borderColor: "border-green-200",
      barBg: "bg-green-100",
      barFill: "bg-sun-green-600",
      width: "w-[90%]",
    },
    {
      name: "Nebulosidade",
  ...
  return (
    <main className="w-full p-4 md:p-6 lg:p-8 space-y-5 bg-[#eeede8] min-h-screen text-sun-text font-sans">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-black/10 pb-5 gap-4">
        ...
        <div className="flex items-center flex-wrap gap-3">
          ...
          >
            <FileText
              size={16}
              className="transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:scale-110 group-hover:text-sun-amber-400"
            />
          ...
          <Link
            href="/rastreador"
            className="group relative flex items-center gap-2 bg-sun-green-400 hover:bg-sun-green-bg-sun-green-600 text-white px-5 py-2.5 rounded-full shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-900/20 hover:ring-2 hover:ring-offset-2 hover:ring-sun-green-600/60 active:scale-95 active:translate-y-0"
          >
            <Calculator
              size={16}
              className="transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:scale-110 group-hover:text-sun-amber-400"
          ...
          >
            <MapPin
              size={16}
              className="text-sun-green-600 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:scale-110 group-hover:text-sun-amber-500"
            />
          ...
          <div className="bg-white border border-black/10 px-5 py-2.5 rounded-full shadow-sm min-w-50 text-center">
            <span className="text-[11px] font-black text-sun-text/80 tracking-wide uppercase">
              {currentDateTime || "Sincronizando..."}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="flex items-center gap-2 bg-white px-4 py-3.5 rounded-lg border border-black/10 shadow-sm">
        ...
        <span className="font-black text-sun-text text-base">
        ...
        <span className="font-black text-[#15803d] text-sm tracking-tight">
          lat -8.33° <span className="text-black/10 mx-1.5">·</span> lon -36.42°{" "}
          <span className="text-black/10 mx-1.5">·</span> alt 768 m
        </span>
      ...
      <Card className="border-black/10 shadow-md rounded-xl overflow-hidden bg-white">
        <CardContent className="p-8 flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="flex-1 space-y-3 text-center md:text-left">
            <h2 className="text-2xl font-black text-sun-text">
              Ótimo para instalação solar!
            </h2>
            <p className="text-[16px] font-bold text-[#4a4944] leading-relaxed max-w-2xl">
              Esta região apresenta alta irradiação, baixa nebulosidade e
              ângulos favoráveis. A instalação de painéis definitivos tem alto
              retorno potencial no Agreste.
            </p>
          </div>
          <div className="flex flex-col items-end shrink-0 bg-green-50/50 p-6 rounded-2xl border border-green-100/50">
            <span className="text-6xl font-black text-[#15803d] leading-none tracking-tighter">
              84
            </span>
            <span className="text-[11px] font-black uppercase tracking-[0.25em] text-[#6b6a64] mt-3">
              viabilidade / 100
            </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {
            label: "Energia captada hoje",
            val: "6.4",
            unit: "kWh / m²",
            delta: "+12% vs. média local",
          },
          {
            label: "Irradiação pico",
            val: "891",
            unit: "W/m² às 11h42",
            delta: "Alta — dia excelente",
          },
          {
            label: "Horas de sol pleno",
            val: "9.1",
            unit: "h estimadas hoje",
            delta: "Acima da média nacional",
          },
          {
            label: "Eficiência de rastreio",
            val: "94%",
            unit: "captação atual",
            delta: "Rastreio biaxial ativo",
          },
        ].map((kpi, i) => (
          <div
            key={i}
            className="bg-white border border-black/5 p-5 rounded-xl shadow-sm flex flex-col justify-center"
          >
            ...
            <p className="text-[14px] font-extrabold text-[#15803d] mt-1 mb-2">
              {kpi.unit}
            </p>
            <p className="text-[11px] font-bold text-[#15803d] bg-green-100/50 self-start px-2 py-0.5 rounded-md border border-green-200/50">
              {kpi.delta}
            </p>
          </div>
        ))}
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-1 border border-black/5 rounded-full p-1 bg-[#eeede8]/50">
                {["Hoje", "Semana", "Mês", "Ano"].map((label, idx) => (
                  <button
                    key={label}
                    className={`text-[11px] px-5 py-2 rounded-full font-black transition-all uppercase tracking-wider ${idx === 0 ? "bg-sun-green-600 text-white shadow-md" : "text-[#6b6a64] hover:bg-black/5"}`}
                  >
                    {label}
                  </button>
                ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {fatores.map((f, i) => (
          <div
            key={i}
            className="bg-white border border-black/5 p-4 rounded-xl shadow-sm flex items-center gap-5"
          >
            {renderFactorIcon(f.icon, f.color, f.bgColor, f.borderColor)}
            <div className="flex-1">
              <p className="text-[11px] uppercase font-black tracking-widest text-[#6b6a64] mb-0.5">
                {f.name}
              </p>
